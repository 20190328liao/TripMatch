...
namespace TripMatch.Services
{
    ...
    {
        private readonly ConcurrentDictionary<string, ConcurrentDictionary<int, UserNotification>> _notifications = new();
        private readonly ConcurrentDictionary<string, ConcurrentDictionary<int, VerificationEmailLog>> _emailLogs = new();
        private int _notifId;
        private int _logId;

        public VerificationEmailLog AddVerificationEmail(string userId, DateTime sentAt, string? note = null)
        {
            var id = Interlocked.Increment(ref _logId);
            var log = new VerificationEmailLog { Id = id, UserId = userId, SentAt = sentAt, Note = note, IsReminded = false };
            var dict = _emailLogs.GetOrAdd(userId, _ => new ConcurrentDictionary<int, VerificationEmailLog>());
            dict[id] = log;
            return log;
        }

        public IEnumerable<VerificationEmailLog> GetAllUnremindedLogsForDate(DateTime date)
        {
            var d = date.Date;
            return _emailLogs.Values.SelectMany(dict => dict.Values)
                .Where(l => l.SentAt.Date == d && !l.IsReminded);
        }

        public void MarkLogReminded(string userId, int logId)
        {
            if (_emailLogs.TryGetValue(userId, out var dict) && dict.TryGetValue(logId, out var log))
            {
                log.IsReminded = true;
            }
        }

        public UserNotification AddNotification(string userId, string message, string? type = null)
        {
            var id = Interlocked.Increment(ref _notifId);
            var n = new UserNotification { Id = id, UserId = userId, Message = message, Type = type, IsRead = false, CreatedAt = DateTime.UtcNow };
            var dict = _notifications.GetOrAdd(userId, _ => new ConcurrentDictionary<int, UserNotification>());
            dict[id] = n;
            return n;
        }
        ...
        {
            if (_notifications.TryGetValue(userId, out var dict))
            {
        ...
        public bool MarkNotificationRead(string userId, int id)
        {
            if (_notifications.TryGetValue(userId, out var dict) && dict.TryGetValue(id, out var n))
            {
                n.IsRead = true;
                return true;
            }
        ...
        public void DismissAll(string userId)
        {
            if (_notifications.TryGetValue(userId, out var dict))
            {
                foreach (var kv in dict) kv.Value.IsRead = true;
            }
        }
    }
    //背景查通知
    public class EmailVerificationReminderService : BackgroundService
    {
        private readonly InMemoryNotificationStore _store;
        private readonly IServiceProvider _services;
        private readonly ILogger<EmailVerificationReminderService> _logger;

        public EmailVerificationReminderService(InMemoryNotificationStore store, IServiceProvider services, ILogger<EmailVerificationReminderService> logger)
        {
            _store = store;
            _services = services;
            _logger = logger;
        ...
        {
            _logger.LogInformation("EmailVerificationReminderService started.");
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    var today = DateTime.UtcNow.Date;
                    var logs = _store.GetAllUnremindedLogsForDate(today).ToList();
                    ...
                    foreach (var log in logs)
                    {
                        try
                        {
                            ...
                            var confirmed = user != null && await userManager.IsEmailConfirmedAsync(user);
                            if (!confirmed)
                            {
                                _store.AddNotification(log.UserId, "您今日收到驗證信，尚未完成驗證，請至信箱完成驗證以啟用完整功能。", "EmailVerificationReminder");
                            }
                            _store.MarkLogReminded(log.UserId, log.Id);
                        }
                        catch (Exception ex)
                        {
                            _logger.LogError(ex, "處理 VerificationEmailLog Id={Id} 失敗", log.Id);
                        }
                    }
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "EmailVerificationReminderService 執行失敗");
                }
